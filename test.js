...
function sendUserParties(socket, user) {
    try {
        ...
        if (user.partyId && parties.has(user.partyId)) {
            ...
            logEvent('파티 갱신', `userId=${user.userId}, partyName=${myParty.partyName}`);
        }
        ...
        for (const [pid, party] of parties.entries()) {
            ...
            // 풀 파티 제외
            const currentCount = party.members.size;
...
function broadcastAllUserPartyList() {
    for (const user of users.values()) {
        const socket = io.sockets.sockets.get(user.socketId);
        if (socket) {
            sendUserParties(socket, user);
        } else {
            user.socketId = null;
        }
    }
...
io.on('connection', socket => {
    ...
    socket.on('leave_party', () => {
        ...
        logEvent('파티 탈퇴', `userId=${userId}, partyId=${party.partyId}`);
    });
    ...
    socket.on('kick_member', data => {
        ...
        if (targetUser) {
            ...
            if (sock) sock.emit('kicked_from_party');
        }
    ...
    socket.on('handle_join_request', data => {
        ...
        // 가입 요청 수락/거절 처리 후 파티장에게 정보 갱신 전송
        const leaderUser = users.get(userId);
        const leaderSocket = io.sockets.sockets.get(leaderUser?.socketId);
        if (leaderSocket) {
            sendUserParties(leaderSocket, leaderUser);
        }
    });
